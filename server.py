...
app = Flask(__name__)
app.config['MAX_CONTENT_LENGTH'] = 40 * 1024 * 1024  # 40 МБ на файл
...
def file_too_large(e):
    return 'Файл слишком большой (максимум 40 МБ на один файл)', 413
...
def upload_file():
    ...
    output_files, input_files, txt_files, errors = [], [], [], []
    ...
    if not output_files:
        for path in input_files: os.remove(path)
    ...
    def cleanup(response):
        try:
            for _, path in output_files: os.remove(path)
            for path in input_files: os.remove(path)
            for _, path in txt_files: os.remove(path)
    ...
    if zip_mode or len(output_files) > 1 or (('pdf' in formats) and ('txt' in formats)):
        ...
        with ZipFile(zip_path, 'w') as zipf:
            for orig_name, path in output_files:
                if 'pdf' in formats: zipf.write(path, f"ocr_{orig_name.rsplit('.',1)[0]}.pdf")
            if 'txt' in formats:
...
def transcribe_audio():
    ...
    input_path = os.path.join(UPLOAD_FOLDER, str(uuid.uuid4()) + "_" + filename)
    f.save(input_path)
    ...
    return send_file(output_path, as_attachment=True, download_name=f"{os.path.splitext(filename)[0]}.docx")
